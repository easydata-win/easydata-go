...
func serve(t *testing.T, replies ...reply) (*Client, *[]seen) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// A string body is written through as it stands: the jsonl and csv
		// exports are not JSON, and encoding them would test a shape the server
		// never sends.
		if text, ok := rep.body.(string); ok {
...
const csvHead = "item_index,page,status,credits_used,input,data,error\n"

func TestExportJoinsCSVPagesUnderOneHeader(t *testing.T) {
	// The server sends the header on every page. A file carrying one every
	// hundred rows is one most tools refuse halfway through.
	c, got := serve(t,
		reply{status: 200, body: csvHead + "0,,succeeded,1,x,{},\n", headers: map[string]string{"X-Next-Cursor": "c1"}},
		reply{status: 200, body: csvHead + "1,,succeeded,1,y,{},\n", headers: map[string]string{"X-Next-Cursor": "c2"}},
		// No entries left: the server echoes back the cursor it was given.
		reply{status: 200, body: csvHead, headers: map[string]string{"X-Next-Cursor": "c2"}},
	)

	var out strings.Builder
	n, err := c.ExportTo(context.Background(), &out, "b-1", ExportCSV, "", 0)
	if err != nil {
	...
	if c := strings.Count(out.String(), "item_index,page,status"); c != 1 {
		t.Errorf("header appears %d times, want 1:\n%s", c, out.String())
	}
	if !strings.Contains(out.String(), "0,,succeeded") || !strings.Contains(out.String(), "1,,succeeded") {
		t.Errorf("rows are missing:\n%s", out.String())
	}
	if q := (*got)[0].query; !strings.Contains(q, "format=csv") {
		t.Errorf("first page did not ask for csv: %q", q)
	}
	if q := (*got)[1].query; !strings.Contains(q, "cursor=c1") {
		t.Errorf("second page did not resume from the cursor: %q", q)
	}
}

func TestExportPageReadsTheCursorFromTheHeader(t *testing.T) {
	// jsonl and csv have no envelope, so the cursor is a header. Reading it
	// from a meta that is not there would page forever from the start.
	c, _ := serve(t, reply{status: 200, body: "{\"item_index\":0}\n",
		headers: map[string]string{"X-Next-Cursor": "c9"}})

	page, err := c.ExportPageAt(context.Background(), "b-1", ExportJSONL, "", 0)
	if err != nil {
		t.Fatalf("ExportPageAt: %v", err)
	}
	if page.NextCursor != "c9" {
		t.Errorf("cursor = %q, want c9", page.NextCursor)
	}
	if !strings.Contains(page.Body, "item_index") {
		t.Errorf("body = %q", page.Body)
	}
}

func TestAnUnknownExportFormatIsRefusedLocally(t *testing.T) {
	c, got := serve(t)
	if _, err := c.ExportPageAt(context.Background(), "b-1", ExportFormat("xlsx"), "", 0); err == nil {
		t.Fatal("an unknown format was accepted")
	}
	if len(*got) != 0 {
		t.Error("an unknown format reached the server")
	}
}

// ------------------------------------------------------------------- account

func TestUpdateAccountSendsOnlyWhatWasGiven(t *testing.T) {
	// Absent and empty are different requests: patching the url alone must not
	// rotate the secret every integration is already verifying against.
	c, got := serve(t, reply{status: 200, body: map[string]any{
		"data": map[string]any{"org_id": "o-1"}, "meta": map[string]any{},
	}})

	var acct map[string]any
	if err := c.UpdateAccount(context.Background(),
		AccountUpdate{WebhookURL: Ptr("https://example.com/hooks")}, &acct); err != nil {
		t.Fatalf("UpdateAccount: %v", err)
	}

	if (*got)[0].method != http.MethodPatch {
		t.Errorf("method = %s, want PATCH", (*got)[0].method)
	}
	body := (*got)[0].body
	if body["webhook_url"] != "https://example.com/hooks" {
		t.Errorf("body = %+v", body)
	}
	if _, ok := body["webhook_secret"]; ok {
		t.Error("an unmentioned secret was sent and would have rotated it")
	}
}

func TestUpdateAccountSendsAnEmptySecret(t *testing.T) {
	c, got := serve(t, reply{status: 200, body: map[string]any{
		"data": map[string]any{"org_id": "o-1"}, "meta": map[string]any{},
	}})

	if err := c.UpdateAccount(context.Background(),
		AccountUpdate{WebhookSecret: Ptr("")}, nil); err != nil {
		t.Fatalf("UpdateAccount: %v", err)
	}
	if v, ok := (*got)[0].body["webhook_secret"]; !ok || v != "" {
		t.Errorf("an empty secret must be sent - it is what asks for a new one: %+v", (*got)[0].body)
	}
}

func TestAnEmptyAccountUpdateNeverReachesTheWire(t *testing.T) {
	c, got := serve(t)
	if err := c.UpdateAccount(context.Background(), AccountUpdate{}, nil); err == nil {
		t.Fatal("an empty update was accepted")
	}
	if len(*got) != 0 {
		t.Error("an empty update reached the server")
	}
}
...
// receiver. Regenerate from the server's signing function with the secret
