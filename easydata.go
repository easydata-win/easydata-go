...
// AccountUpdate changes where completions are delivered and what signs them.
//
// Both fields are pointers because "not mentioned" and "set to empty" are
// different requests: patching only the URL must not rotate the secret every
// integration is already verifying against. Ptr is the way to write one.
type AccountUpdate struct {
	// WebhookURL is the standing destination for a completion. Empty clears it.
	WebhookURL *string `json:"webhook_url,omitempty"`
	// WebhookSecret is what signs deliveries. Empty has the server generate
	// one; a value of your own must be at least 32 characters.
	WebhookSecret *string `json:"webhook_secret,omitempty"`
}

// Ptr is the one-liner for building an AccountUpdate field.
func Ptr[T any](v T) *T { return &v }

// UpdateAccount sets the standing webhook destination, the signing secret, or
// both, and returns the account as GET /account would.
//
// It needs a key with the write scope: it is the only write on the account, and
// a read-only key must not be able to move where deliveries go.
//
//	acct := map[string]any{}
//	err := ed.UpdateAccount(ctx, easydata.AccountUpdate{
//		WebhookURL: easydata.Ptr("https://example.com/hooks/easydata"),
//	}, &acct)
func (c *Client) UpdateAccount(ctx context.Context, up AccountUpdate, dst any) error {
	if up.WebhookURL == nil && up.WebhookSecret == nil {
		return &Error{Type: ErrTypeInvalidRequest, Message: "send webhook_url, webhook_secret, or both"}
	}
	_, err := c.do(ctx, http.MethodPatch, "/account", up, nil, "", dst)
	return err
}

// ExportFormat is a flat rendering of the results cursor, for a caller who
// wants a file rather than objects.
type ExportFormat string

const (
	// ExportJSONL is one entry per line, each identical to a cursor entry.
	ExportJSONL ExportFormat = "jsonl"
	// ExportCSV is identity, outcome and cost as columns, with the record left
	// whole in a JSON cell - flattening a profile into columns would mean
	// choosing a shape every caller then works around.
	ExportCSV ExportFormat = "csv"
)

// ExportPage is one read of the cursor in a flat format.
//
// NextCursor comes from the X-Next-Cursor HEADER rather than from a meta,
// because neither format has an envelope to carry one. The server echoes the
// cursor it was given when a page held no entries, which is how ExportTo knows
...
// ExportPageAt reads one page of the cursor as jsonl or csv.
//
// ExportTo is the loop you usually want. This is the layer under it, for a
// caller that owns its own paging. Note that every CSV page carries its own
// header row: concatenating pages without dropping it is a file with a header
// every hundred rows.
func (c *Client) ExportPageAt(ctx context.Context, batchID string, format ExportFormat,
	cursor string, pageSize int) (*ExportPage, error) {

	// Checked here rather than left to the server, which falls back to JSON for
	// a format it does not know: a typo would come back as an envelope with no
	// X-Next-Cursor, which reads as a batch of one page.
	if format != ExportJSONL && format != ExportCSV {
		return nil, &Error{Type: ErrTypeInvalidRequest, Message: fmt.Sprintf("unknown export format %q: jsonl or csv", format)}
	}
	if pageSize <= 0 {
		pageSize = 500
	}
	...
	raw, header, err := c.send(ctx, http.MethodGet, "/batches/"+batchID+"/results", nil, q, "", accept)
	if err != nil {
		return nil, err
	}
	return &ExportPage{Body: string(raw), NextCursor: header.Get("X-Next-Cursor")}, nil
}

// ExportTo writes a batch's results to w as jsonl or csv, and returns how many
// bytes it wrote.
//
// It exports what is READABLE NOW rather than waiting: neither format carries
// the batch status, so a client cannot tell a finished batch from a busy one
// without asking again. Call Wait first for the whole batch, or keep the cursor
// this stops at.
//
// The CSV header is written once. The server sends it on every page, and a
// concatenation that kept them all is a file most tools refuse halfway through.
func (c *Client) ExportTo(ctx context.Context, w io.Writer, batchID string,
	format ExportFormat, cursor string, pageSize int) (int64, error) {

	var written int64
	for first := true; ; first = false {
		page, err := c.ExportPageAt(ctx, batchID, format, cursor, pageSize)
		if err != nil {
			return written, err
		}

		// The server echoes the cursor back when a page held no entries, which
		// is the only end-of-results signal these two formats have.
		if !first && page.NextCursor == cursor {
			return written, nil
		}

		body := page.Body
		if !first && format == ExportCSV {
...
func (c *Client) do(ctx context.Context, method, path string, body any, q url.Values,
	...
	raw, _, err := c.send(ctx, method, path, body, q, idempotencyKey, "application/json")
	if err != nil {
		return meta{}, err
	}
...
// send performs one call, with retries, and returns the body and the response
// headers as they arrived.
//
// It exists because two things ride on this transport and only one of them is
// JSON: do unmarshals an envelope, and ExportPageAt reads jsonl or csv, whose
// cursor is a HEADER because those formats have no envelope to carry a meta in.
// A second retry loop for the second case is a second place for the rules about
// what may be repeated to drift.
func (c *Client) send(ctx context.Context, method, path string, body any, q url.Values,
	idempotencyKey, accept string) ([]byte, http.Header, error) {
	...
	if body != nil {
		...
		if payload, err = json.Marshal(body); err != nil {
			return nil, nil, fmt.Errorf("easydata: encoding request: %w", err)
		}
	}
	...
	for attempt := 0; ; attempt++ {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("easydata: building request: %w", err)
		}
		...
		if resp.StatusCode >= 200 && resp.StatusCode < 300 {
			if readErr != nil {
				return nil, nil, fmt.Errorf("easydata: reading response: %w", readErr)
			}
