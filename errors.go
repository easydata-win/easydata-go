...
const (
	...
	ErrTypeInsufficientScope   = "insufficient_scope"
...
var (
	...
	// ErrInsufficientCredits is 402: the prepaid balance will not cover the
	// request. Its own sentinel because its own remedy - ErrQuotaExhausted is
	// fixed by the month turning over and this one is fixed by buying credits,
	// so code that retries the first on a schedule would retry this forever.
	ErrInsufficientCredits = &Error{Type: ErrTypeInsufficientCredits}
	ErrEmailUnverified     = &Error{Type: ErrTypeEmailUnverified}
	ErrInsufficientScope   = &Error{Type: ErrTypeInsufficientScope}
...
// AsErrorIs is errors.As with the destination passed in, for a caller that
// already has a *Error variable to fill.
func AsErrorIs(err error, dst **Error) bool { return errors.As(err, dst) }
