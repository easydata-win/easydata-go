...
Six events: `batch.started`, `batch.result` (one per row), `batch.completed`
and `batch.failed` are about the work; `credits.purchased` and `credits.low` are
about the account paying for it, and they are how a pipeline stopped by a 402
learns it can start again. The fields are in `d.Data`, one level down -
`CompletionPayload`, `StartedPayload`, `ResultPayload`, `PurchasePayload` or
`LowPayload` according to `d.Event`.
...
ed.UpdateAccount(ctx, easydata.AccountUpdate{ // needs a write-scoped key
    WebhookURL: easydata.Ptr("https://example.com/hooks/easydata"),
}, &account)
ed.Usage(ctx, "2026-09-01", "2026-09-30", "", &usage)
```

`UpdateAccount` moves the standing webhook destination and rotates the signing
secret. Its fields are pointers because absent and empty are different requests:
passing only `WebhookURL` leaves the secret every integration is verifying
against alone, and `Ptr("")` for the secret asks for a new one, which
invalidates the old signature immediately.

## A file instead of records

The cursor also serves `jsonl` and `csv`, for a caller who wants a file:

```go
f, _ := os.Create("batch.csv")
defer f.Close()
n, err := ed.ExportTo(ctx, f, batchID, easydata.ExportCSV, "", 0)
```

The CSV leaves each record whole in a JSON cell rather than flattening a profile
into columns, and the header is written once - the server sends it on every
page. `ExportTo` writes what is READABLE NOW rather than waiting, because
neither format carries the batch status: call `ed.Wait` first for the whole
batch. `ExportPageAt` is the single page underneath, for a caller that owns its
own paging.

Operations: `OpProfilesEnrich`, `OpProfilesEnrichDetailed`,
`OpProfilesActivity`, `OpProfilesPosts`, `OpProfilesComments`,
`OpProfilesReactions`, `OpCompaniesEnrich`, `OpPostsEnrich`,
`OpSalesSearchPeople`, `OpSalesSearchEmployees`, `OpSalesSearchCompanies`.

`OpProfilesEnrichDetailed` is `OpProfilesEnrich` plus the optional sections:
`SubmitOptions.IncludeCounts` for follower and connection counts,
`IncludeExperienceDetail` for what each role actually says,
`IncludeSkillDetails` for endorsement counts, `IncludeInterests` for what the
person follows, and `IncludeRecommendations` for written recommendations in
both directions. Each is one more request to LinkedIn and costs +0.5 per
target, charged only when the section answered, and at least one of them is
required.
