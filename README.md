...
`SubmitOptions.IncludeCounts` for follower and connection counts, and
`IncludeExperienceDetail` for what each role actually says. Each is one more
request to LinkedIn and costs +0.5 per target, charged only when the section
answered, and at least one of them is required.
